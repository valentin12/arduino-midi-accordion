...
##Technical
The extension is based on an Arduino Mega 2560 and programmed in C++, all the code can be found in the src/ Folder. The USB-port is used to get energy, but the MIDI signals are sent with baudrate 9600 on `Serial` so they can be accessed over USB, too. On the DIN connector (`Serial1` - TX1) are the MIDI signals sent with the baudrate 31250.
